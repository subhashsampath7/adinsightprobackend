...
const sendLicenseKeyEmail = async(email, name, licenseKey, expiresAt) => {
    try {
        const htmlContent = `
            ...
            <body>
                <div class="container">
                    ...
                    <div class="footer">
                        <p>© 2026 Google Ads Transparency. All rights reserved.</p>
                    </div>
                </div>
            </body>
            </html>
        ...
        console.error('❌ Failed to send license key email:', error.response ? error.response.data : error.message);
    }
};
...
const sendPaymentDeclinedEmail = async(email, name, reason) => {
    try {
        ...
        console.error('❌ Failed to send payment declined email:', error.response ? error.response.data : error.message);
    }
};
